...
A native macOS app that replicates the functionality of X.com (formerly Twitter), built entirely with **Swift** and **SwiftUI**. Everything runs locally against rich sample data, so the whole app is interactive out of the box — no server, no accounts, no network.
...
The feature set was researched against X.com as of 2026 and mapped onto native macOS equivalents:
...
└── Sources/PingIt/
    ├── PingItApp.swift         # @main entry, appearance handling, AppKit activation
    ...
    ├── ContentView.swift       # NavigationSplitView shell, sidebar, shared components
    ...
    ├── MessagesView.swift      # Split-pane DMs
    ├── ProfileView.swift       # Profiles + edit profile sheet
    └── SidebarExtras.swift     # Bookmarks, Lists, Communities, Settings
```
